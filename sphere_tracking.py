...
def circle_detection(segmentations, NUMBER_OF_PROJECTIONS):
    ...
    for segmentation_num, segmentation in enumerate(segmentations):
        ...
        for i, mask in enumerate(segmentation):
            ...
            # image.show()
            image.save(f'Segmentations/{segmentation_num}_{i}.png')
            ...
            if circles is not None:
                ...
                if boolean_segmentation_array[param[1], param[0]]:
                    ...
                    projection_idx.append(segmentation_num)
...
def main():
    ...
    with open('CV_xy_CoM', 'w') as file:
        file.write(str_CV_xy_CoM)
    with open('SAM_xy_CoM', 'w') as file:
        file.write(str_SAM_xy_CoM)
    with open('CV_radii', 'w') as file:
        file.write(str_CV_radii)
    with open('SAM_radii', 'w') as file:
        file.write(str_SAM_radii)
    with open('projection_idx', 'w') as file:
